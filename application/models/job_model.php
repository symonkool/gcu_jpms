...
{	
	...
	{
		return $this->db->select('location.*')->from('location')->get()->result_array();
	}
	...
	{   
		...
		$this->db->where("(title LIKE '%{$data['search']}%' OR company LIKE '%{$data['search']}%' OR description LIKE '%{$data['search']}%' OR responsibilities LIKE '%{$data['search']}%' OR skills LIKE '%{$data['search']}%' OR duration LIKE '%{$data['search']}%')");
		if(isset($data['city']))
		{
