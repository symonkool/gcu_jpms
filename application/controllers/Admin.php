...
class Admin extends CI_Controller 
{
	...
	{	
		...
		$userole='Admin';
		$where = array('email'=>$email,'password'=>$password,'role'=>$userole);
	...
	function register()
	{
		...
		$data = array('name'=>$name,'email'=>$email,'password'=>$password,'role'=>'User','date_created'=>date('Y-m-d H:i:s'));
	...
	function adminregister()
	{
		...
		$email = $this->security->xss_clean($this->input->post('r_email'));
		$password = md5(md5($this->security->xss_clean($this->input->post('r_pass'))));
		$data = array('name'=>$name,'email'=>$email,'password'=>$password,'role'=>'Admin','date_created'=>date('Y-m-d H:i:s'));
		$res= $this->user_model->register($data);
		if($res)
		{
			$this->session->set_userdata('login_status','register_success');
